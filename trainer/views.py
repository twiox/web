...
def abrechnungstable(request):
    table, created = Trainer_table.objects.get_or_create(
        trainer=request.user.trainer, active=True
    )
    sessions = Session.objects.filter(trainer=request.user.trainer)
    if created:
        for sess in sessions.all():
            Table_entry.objects.create(
                table=table,
                day=sess.day,
                group=sess.agegroup,
                start=sess.start_time.strftime("%H:%M"),
                end=sess.end_time.strftime("%H:%M"),
            )

    entries = Table_entry.objects.filter(table=table).all()
    tables = Trainer_table.objects.filter(
        trainer=request.user.trainer, active=False
    ).order_by("-id")[:10]
    return render(
        request,
        "trainer/abrechnung_form.html",
        context={"tables": tables, "entries": entries},
    )
...
def add_week(request):
    ...
    data = {}
    for sess in sessions.all():
        temp = Table_entry.objects.create(
            table=table,
            day=sess.day,
            group=str(sess.agegroup),
            start=sess.start_time.strftime("%H:%M"),
            end=sess.end_time.strftime("%H:%M"),
        )
        entries.append(temp)
...
def add_row(request):
    table = Trainer_table.objects.get(trainer=request.user.trainer, active=True)
    obj = Table_entry.objects.create(
        table=table, day="Mo", start="17:00", end="19:00", group="A"
    )
    data = {}
    data[obj.pk] = {
        "day": obj.day,
...
def send_email(table, hours):
    ...
    with codecs.open(table.final_file.path, "w", encoding="windows-1252") as outfile:
        ...
        print(
            "\n".join(
                [str(entry) for entry in Table_entry.objects.filter(table=table).all()]
            ),
            file=outfile,
        )
        print("--------", file=outfile)
        print(
            f"Stunden: {hours}\tTAE/Stunde: {user.trainer.salary} €\tGesamt: {sal:.2f} €",
            file=outfile,
        )
    ...
    try:
        email = EmailMessage(
            mail_subject, message, to=[to_email], cc=[table.trainer.trainer_email]
        )
...
def send_table(request):
    ...
    table.final_file.save(
        f"{datetime.today().strftime('%Y-%m-%d')}_Abrechnung_{request.user.first_name}_{request.user.last_name}.txt",
        ContentFile("Trainerabrechnung"),
    )
    table.save()
...
class AbrechnungstableDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def test_func(self):
        ...
        return (
            hasattr(self.request.user, "chairman")
            or table.trainer == self.request.user.trainer
        )
...
def register_trainer(request):
    choices = [
        (x.pk, f"{x.first_name} {x.last_name} ({x.profile.member_num})")
        for x in User.objects.filter(trainer__id__isnull=True)
    ]
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    "Trainer angelegt. Gruppe wurde auf T gesetzt",
                )
                return redirect("register_trainer")
        else:
            return render(
                request,
                "trainer/trainer_form.html",
                context={"form": form, "choices": choices},
            )
    else:
        form = TrainerCreationForm()
    return render(
        request, "trainer/trainer_form.html", context={"form": form, "choices": choices}
    )
    ...
    if request.method == "POST":  # if the form is filled out
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                f"Trainerstatus für {user.first_name} entfernt. Gruppe wurde auf {group.group_id} gesetzt",
            )
