...
urlpatterns = [
    ...
    path(
        "aktiviere/<str:uidb64>/<str:token>/neu",
        user_views.resend_activate,
        name="resend_activate",
    ),
    path(
        "passwort_zurücksetzen/",
        user_views.PasswordResetView.as_view(
            template_name="user_handling/password_reset.html"
        ),
        name="password_reset",
    ),
    path(
        "passwort_zurücksetzen/bestätigen/<str:uidb64>/<str:token>/",
        user_views.PasswordResetConfirmView.as_view(
            template_name="user_handling/password_reset_confirm.html"
        ),
