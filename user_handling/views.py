...
def register(request):
    ...
    if request.method == "POST":  # if the form is filled out
        ...
        if form.is_valid() and form2.is_valid():  # and the form is valid
            ...
            real_user.username = form.cleaned_data.get(
                "first_name"
            ).lower() + form2.cleaned_data.get("member_num")
            ...
            message = render_to_string(
                "user_handling/acc_active_email.html",
                {
                    "user": real_user,
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                "Account angelegt. Das Mitglied bekommt eine Email",
            )
    ...
    return render(
        request,
        "user_handling/register.html",
        context={
            "form": form,
            "form2": form2,
            "current_member": get_ordering(current_member),
            "ref": ref,
        },
    )
...
def activate(request, uidb64, token):
    ...
    if (
        ...
        return render(
            request,
            "user_handling/activate.html",
            context={"user": user, "uid": uid2, "token": token2},
        )
    else:
        return render(
            request,
            "user_handling/activate_fail.html",
...
def resend_activate(request, uidb64, token):
    ...
    try:
        ...
        email.send()
        messages.add_message(
            request, messages.SUCCESS, "Neuer Aktivierungslink verschickt"
        )
    except:
        messages.add_message(
            request,
            messages.SUCCESS,
            "FEHLER: Der Benutzer existiert nicht. Bitte wende dich an den Vorstand",
        )
...
def member_update(request, pk):
    ...
    if request.method == "POST":  # if the form is filled out
        ...
        if form.is_valid() and form2.is_valid():  # and the form is valid
            ...
            real_user.save()  # save the entries
            messages.add_message(
                request, messages.SUCCESS, "Account erfolgreich bearbeitet"
            )
    ...
    return render(
        request, "user_handling/register.html", context={"form": form, "form2": form2}
    )
...
class ChairmanCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["choices"] = [
            ...
            for x in User.objects.filter(chairman__id__isnull=True)
        ]
        return context
...
class PasswordResetView(PasswordResetView):
    def get_success_url(self):
        ...
        messages.add_message(
            self.request,
            messages.SUCCESS,
            "Email verschickt. Bitte schau in deinen Posteingang",
        )
...
class PasswordResetConfirmView(PasswordResetConfirmView):
    def get_success_url(self):
        ...
        messages.add_message(
            self.request,
            messages.SUCCESS,
            "Passwort erstellt. Du kannst dich nun einloggen",
        )
...
def get_participants_emails(request):
    ...
    if "type" in data:
        ...
        for part in MemberParticipant.objects.filter(event=event):
...
def member_detail_update(request, pk):
    ...
    if data["add_email_email"] and data["add_email_title"]:
        mail = AdditionalEmail.objects.create(
            user=user, title=data["add_email_title"], email=data["add_email_email"]
        )
        mail.save()
    try:
        user.save()
